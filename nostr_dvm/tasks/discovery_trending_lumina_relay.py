...
class TrendingLUMINARelay(DVMTaskInterface):
    ...
    async def process(self, request_form):
        ...
        url = "https://relay.lumina.rocks/api/trending/kind20"
        try:
            response = requests.get(url)
            response_json = response.json()
            result_list = []
            i = 0
            if len(response_json["trending"]) > 0:
                for note in response_json["trending"]:
                    i += 1
